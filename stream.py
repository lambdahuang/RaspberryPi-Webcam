...
class CameraStream:
    ...
    def __init__(self, connection, storage_directory, shotcut_interval, logger):
        ...
        self._camera_name = struct.unpack(
            '<50s',
            connection.read(struct.calcsize('<50s')))[0].rstrip('\x00')
        ...
        self.status = 'Active'
        self._logger = logger
        self._main_thread = threading.Thread(self.stream_receiver, (connection.makefile('rb'),))
    ...
    def _take_shotcut(self):
        if time.time() - self._last_shotcut_time >= self.shotcut_interval:
        ...
        try:
            while True:
                ...
                (sender_time_stamp, image_len) = struct.unpack(
                    '<1dL',
                    self._connection.read(struct.calcsize('<1dL'))
                )
                ...
                self._put_text_on_image("{.2f}kps\t{.2f}fps%s".format(
                    bandwidth,
                    frame_per_second,
